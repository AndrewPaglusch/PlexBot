...
def api_query_movie(query)
  url = URI.parse("#{@radarr_url}:#{@radarr_port}#{@radarr_context}/api/#{@radarr_api_key}/#{URI.escape(query)}")

  request = Net::HTTP.new(url.host, url.port)

  #Set timeout and scheme
  request.use_ssl = url.scheme == 'https'
  request.read_timeout = @radarr_timeout
  request.open_timeout = @radarr_timeout
